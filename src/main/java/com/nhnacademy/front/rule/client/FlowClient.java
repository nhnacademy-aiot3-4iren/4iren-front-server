...
public interface FlowClient {
    ...
    //플로우 빌드 폼 호출
    @GetMapping("/{room-id}/flows/form")
    ResponseEntity<FlowBuildFormResponse> buildForm(
            @PathVariable("room-id") Long roomId
    );

    //플로우 생성
    @PostMapping("/{room-id}/flows")
    ...
    @GetMapping("/{room-id}/flow-templates/{template-id}")
    ...
    //플로우 수정
    @PutMapping("/{room-id}/flows/{flow-id}")
    ...
    //플로우 삭제
    @DeleteMapping("/{room-id}/flows/{flow-id}")
    ...
    //플로우 활성화/비활성화 설정
    @PatchMapping("/{room-id}/flows/{flow-id}/active")
